...
class TunnelManagementHttpClient {
    ...
    async createTunnel(tunnel, options) {
        ...
        for (let i = 0; i <= 3; i++) {
            try {
                const result = (await this.sendTunnelRequest('PUT', tunnel, manageAccessTokenScope, undefined, "forceCreate=true", options, this.convertTunnelForRequest(tunnel), undefined, true));
                preserveAccessTokens(tunnel, result);
                parseTunnelDates(result);
                return result;
            }
    ...
    async updateTunnel(tunnel, options) {
        const result = (await this.sendTunnelRequest('PUT', tunnel, manageAccessTokenScope, undefined, "forceUpdate=true", options, this.convertTunnelForRequest(tunnel)));
        preserveAccessTokens(tunnel, result);
        parseTunnelDates(result);
        return result;
    }
    ...
    async createTunnelPort(tunnel, tunnelPort, options) {
        ...
        const result = (await this.sendTunnelRequest('PUT', tunnel, managePortsAccessTokenScopes, path, "forceCreate=true", options, tunnelPort));
        if (tunnel.ports) {
    ...
    async updateTunnelPort(tunnel, tunnelPort, options) {
        ...
        const path = `${portsApiSubPath}/${portNumber}`;
        tunnelPort = this.convertTunnelPortForRequest(tunnel, tunnelPort);
        const result = (await this.sendTunnelRequest('PUT', tunnel, managePortsAccessTokenScopes, path, "forceUpdate=true", options, tunnelPort));
